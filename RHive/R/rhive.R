...
rhive.init <- function(hive=NULL, libs=NULL, hadoop_home=NULL, hadoop_conf=NULL, hlibs=NULL, verbose=FALSE) {
  ...
  if (is.null(libs)) {
    libs <- sprintf("/%s/lib", hive)
  }

  if (verbose) {
    cat(sprintf("Detected hive=%s and libs=%s\n", hive, libs))
  }

  if (is.null(hadoop_home)) {
    hadoop_home <- Sys.getenv("HADOOP_HOME")
  }

  if (is.null(hadoop_conf)) {
    hadoop_conf <- Sys.getenv("HADOOP_CONF_DIR")
  }

  if (hadoop_conf == "") {
    hadoop_conf <- sprintf("/%s/conf", hadoop_home)
  }
  ...
  if (hadoop_home == "") {
    ...
    if (is.null(hlibs)) {
      hlibs <- sprintf("/%s/lib", hadoop_home)
    }
...
rhive.exist.table <- function(tablename, hiveclient=rhive.defaults('hiveclient')) {
  ...
  if (class(tablelist) == "try-error") {
    stop("fail to execute 'rhive.list.tables()'")
  }
